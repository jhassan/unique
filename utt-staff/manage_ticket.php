        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                ...
                <form id="SearchForm" action="action.php"  method="post">
                            <input type="hidden" name="action" value="UpdateUserTickets">
                <div class="col-lg-12">
                            ...
                            <div class="dataTable_wrapper">
                                <div class="form-group col-lg-3 p-l-0">
                                    ...
                                    <select name="user_id" id="user_id" class="form-control" style="">
                                        ...
                                        $SQL = "SELECT user_id, user_name FROM tbluser ORDER BY user_name";            
                                ...
                                <div class="form-group col-lg-2 p-l-0">
                                 <button style="width: 150px; margin-left: 15px; margin-top: 24px;" id="" type="submit" class="btn btn-block btn-primary">Update Ticket</button>   
                                </div>
                                ...
                                <table class="table table-responsive table-bordered table-hover">
                                    ...
                                    <?php
                                        ...
                                        if(empty($user_id))
                                        {   
                                         $SQL = "SELECT * FROM tblissuerefund ORDER BY id DESC LIMIT {$startpoint} , {$per_page}"; 
                                        }
                                         else
                                         {
                                             $SQL = "SELECT * FROM tblissuerefund WHERE user_id = '".$user_id."' ORDER BY id DESC LIMIT {$startpoint} , {$per_page}";
                                         }      
                                         //echo $SQL;
                                         //$result = MySQLQuery($SQL);
                                    	...
                                    	//$SQL = "SELECT * FROM tblissuerefund ORDER BY id DESC";			
										 ...
										 while($row = mysqli_fetch_array($result)) { // ,MYSQL_ASSOC
                                            ...
                                            if($ticket_status == 1)
                                                ...
                                                $limit_exceed_selected = "selected='selected'";
                                            else if($ticket_status == 11)
                                                $flight_check_in_selected = "selected='selected'";
                                            ...
                                            if($row['vendor_status'] == 1 && $_SESSION["user_type"] == 2)
                                                ...
                                                $vendor_disabled = "";
                                            if($row['update_status_date_time'] == "0000-00-00 00:00:00")
                                                $update_status_date_time = "N/A";
                                            else
                                                $update_status_date_time = date("dMy H:i", strtotime($row['update_status_date_time']));
                                            // Disable show dialog button
                                            //echo $ticket_status ."----". $row['vendor_id'];
                                            if($_SESSION['user_type'] == 2 && $ticket_status != 0 && $row['vendor_id'] != 0)
                                            {
                                                $disabled_dialog = "";
                                                $disabled_print_button = "";
                                                $print_anchor = "href='ticket_print.php?id=".$row['id']."'";
                                                $print_anchor_blank = "_blank";
                                                $disabled_img = "disabledimg";
                                            }
                                            elseif($_SESSION['user_type'] == 2 && $ticket_status == 0 && $row['vendor_id'] == 0)
                                            {
                                                $disabled_dialog = "modal";
                                                $disabled_print_button = "disabled='disabled'";
                                                $print_anchor = "";
                                                $print_anchor_blank = "";
                                                $disabled_img = "";
                                            }
									...
									?>
                                        <tr class="odd gradeX" id="DelID_<?php echo $row['id'];?>">
                                            ...
                                            <td class="left"><?php echo $update_status_date_time;?></td>
                                            ...
                                            <td class="left" id="Created_By_User_<?php echo $row['id'];?>"><?php echo UserName($row['user_id']);?></td>
                                            ...
                                            <td class="left" style="padding-top: 0;padding-bottom: 0;">
                                                <?php echo $arrTicketStatus[$ticket_status - 1];?>
                                                <select name="ticket_status" id="ticket_dropdown_<?php echo $row['id'];?>" <?php echo $disabled; ?> class="get_payment_status form-control valid hide" onchange="ChangePaymentStatus(<?php echo $row['id'];?>, this.value, <?php echo $row['user_id'];?>)">
                                                  ...
                                                  <option value="10" <?php echo $limit_exceed_selected; ?>>Limit Exceeded</option>
                                                  <option value="11" <?php echo $flight_check_in_selected; ?>>Flight Check-in</option>
                                              </select>
                                            </td>
                                            <td class="left" style="padding-top: 0;padding-bottom: 0;">
                                                <?php echo GetVendorName($row['vendor_id']);?>
                                                <select name="vendor_id" id="view_vendor_<?php echo $row['id'];?>" <?php echo $vendor_disabled; ?> class="form-control valid hide" onchange="UpdateVendor(<?php echo $row['id'];?>, this.value)">
                                            ...
                                            <td class="left">
                                                ...
                                                &nbsp;&nbsp;&nbsp;<a class='cursor'><img id="<?php echo $row['id'];?>" data-toggle="<?php echo $disabled_dialog; ?>" data-target=".bs-example-modal-lg14" class="edit_action bs-example-modal-lg144 <?php echo $disabled_img; ?>" src="../images/action-edit.png" height="16" width="16" alt="Edit Action"></a>
                                                <?php } if($_SESSION["user_type"] == 1) { ?>
                                            ...
                                            <td class="left hide" id="Available_Balance_<?php echo $row['id'];?>"><?php echo ClientAvailBalance($row['user_id']);?></td>
                                            <td class="left hide"><a target="_blank" href="http://toursview.com/unique/account_statement.php?admin_user_id=<?php echo $row['user_id']; ?>">URL</a></td>
                                            <td class="left hide" id="Credit_Limit_<?php echo $row['id'];?>"><?php echo ClientCreditLimit($row['user_id']);?></td>
                                            <?php
                                            if(!empty($staff_permissions) && in_array('62', $staff_permissions))
                                            {
                                            ?>
                                            ...
                                            <?php } ?>
                                            <td class="left"><a target="<?php echo $print_anchor_blank; ?>" <?php echo $print_anchor; ?>><button <?php echo $disabled_print_button; ?> type="button" class="btn btn-block btn-primary">Print</button></a></td>
                                        </tr>
									<?php } } else {
                                ...
                                if(empty($user_id))
                                {   
                                    $count_query = "tblissuerefund ORDER BY id DESC";   
                                }
                                 else
                                 {
                                    $count_query = "tblissuerefund WHERE (user_id = '".$user_id."') ORDER BY id DESC";
                                 }
                                if(empty($user_id))
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h5 class="modal-title p-l-10" id="pnr_created_by_info"></h5>
      </div>
      <div class="modal-body" align="center" style="padding: 0px;">
        ...
        <div class="form-group m-b-0 col-lg-4 m-t-10">
            ...
            <input class="form-control" placeholder="%" name="clint_psf_percent" id="client_percent_rec_comm" value="" maxlength="6" type="text" style="width: 70px; float: left; clear: both; margin-right: 0;">
            <input class="form-control" placeholder="" name="clint_psf_percent_value" id="client_rec_comm_total" value="" maxlength="6" type="text" style="width: 70px; margin-left: 11px; float: left;">
        </div>
        <div class="form-group m-b-0 col-lg-4 m-t-10">
            <label style="float: left;">Franchise Comm.</label>
            <input class="form-control" placeholder="%" name="clint_psf_percent" id="ven_percent_rec_comm" value="" maxlength="6" type="text" style="width: 70px; float: left; clear: both; margin-right: 0;">
            <input class="form-control" placeholder="" name="clint_psf_percent_value" id="vendor_rec_comm_total" value="" maxlength="6" type="text" style="width: 70px; margin-left: 11px; float: left;">
        </div>
        <div class="clear"></div>
        <div class="form-group m-b-0 col-lg-4 m-t-10">
        ...
        <div class="form-group col-lg-4 m-b-10 m-t-10">
            <label style="float: left;">Mode</label>
            <div class="clear"></div>
            <label style="float: left;" id="mode_value">Mode</label>
        </div>
        <div class="clear"></div>
        <div class="form-group col-lg-4 m-b-10 m-t-10">
            <label style="float: left;">TICKET STATUS</label>
        <select name="ticket_status" id="ticket_status" class="get_payment_status form-control valid required">
          <option value="0">Select Status</option>
          <option value="1">Prossess</option>
          <option value="9">In Prossess</option>
        ...
        <div class="form-group col-lg-4 m-b-10 m-t-10">
            <label style="float: left;">Available Balance</label>
            <div class="clear"></div>
            <label style="float: left;" id="view_available_balance"></label>
        </div>
        <div class="clear"></div>
        <div class="form-group col-lg-4 m-b-10 m-t-10">
            <label style="float: left;">Select Vendor</label>
           <select name="vendor_id" id="view_vendor" class="form-control valid required">
        ...
        <div class="form-group col-lg-4 m-b-10 m-t-10">
            <label style="float: left;">Credit Limit</label>
            <div class="clear"></div>
            <label style="float: left;" id="view_credit_limit"></label>
        </div>
  ...
  <script>
    $(document).ready(function() {
        ...
        $("#client_percent_rec_comm").keyup(function (){
            ...
            if(client_percent_rec_comm != "" && client_percent_rec_comm != null && client_percent_rec_comm != "NaN")
            {
                ...
                var client_rec_comm_total = parseFloat(client_percent_rec_comm) / 100 * basic_fare;
                if(client_rec_comm_total != "" && client_rec_comm_total != null && client_rec_comm_total != "NaN")
                {
                   ...
                   // alert(hdn_actual_fare_total+"***"+client_rec_comm_total);
                    ...
                    var total_amount = parseFloat(hdn_actual_fare_total) + parseFloat(client_rec_comm_total); 
        ...
        $("#CreateCommission").click(function (){
            ...
            var view_vendor = $("#view_vendor").val();
            if(ticket_status == "0")
            {
                alert("Please select Ticket Status!");
                return false;
            }
            else if(view_vendor == "0")
            {
                alert("Please select Vendor!");
                return false;
            }
            else if($("#hdn_receivable_charges").val() == "0.00")
            {
                alert("Receivable Charges cannot be empty!");
                return false;
            }
            else
            {
                ...
                form_data.append('view_vendor', view_vendor);
                ...
                $.ajax({
                            ...
                            success: function(response){
                                ...
                                //.bs-example-modal-lg14console.log(response); return false;
                                //$("#text_comments").html('');
                                //$("#text_comments").html(response);
                                //console.log(response); return false;
                                window.location.href = 'view_ticket';
                                window.open(
                                  '../unique/account_statement.php?admin_user_id='+response,
   ...
   <input type="hidden" id="currentID" value="" />
    ...
    $(document).ready(function() {
        ...
        // update drop down selected in dialog
        $(".bs-example-modal-lg144").click(function(){
            var id = $(this).attr("id");
            //alert(id);
        });
        ...
        $(document).on('click','.edit_action',function(e){
            var ticket_status = $("#ticket_status").val();
            // if(ticket_status == "")
            //     $("#ticket_status option:eq(0)").attr("selected","selected");
                //$("#ticket_status option:first").attr('selected','selected');
            $('#add_commissions_form')[0].reset();
            var ID = $(this).attr("id");
            $("#edit_action_id").val(ID);
            var form_data = new FormData();   
            var mode_type = $("#ModeType_"+ID).html();  
            var view_available_balance = $("#Available_Balance_"+ID).html();  
            var view_credit_limit = $("#Credit_Limit_"+ID).html();  
            var pnr_user = $("#PNR_"+ID).html();            
            var created_by = $("#Created_By_User_"+ID).html();
            var pnr_created_by_info = "<b>PNR</b>: "+pnr_user+", <b>Created By</b>: "+created_by+"";
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    $("#ticket_status").val(obj.ticket_status);
                    $("#view_vendor").val(obj.vendor_id);
                    $("#mode_value").html(mode_type);
                    $("#view_available_balance").html(view_available_balance);
                    $("#view_credit_limit").html(view_credit_limit);
                    $("#pnr_created_by_info").html(pnr_created_by_info);
                    // window.location.href = "view_ticket";
                    // window.open(
